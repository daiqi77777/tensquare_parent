...
public class ReplyService {
	 ...
	 * @return
	 */
	public Page<Reply> findSearch(Map<String, Object> whereMap, int page, int size) {
	 ...
	 * @return
	 */
	public List<Reply> findSearch(Map<String, Object> whereMap) {
		Specification<Reply> specification = createSpecification(whereMap);
	 ...
	 * @return
	 */
	private Specification<Reply> createSpecification(Map<String, Object> searchMap) {

		return new Specification<Reply>() {

			private static final long serialVersionUID = 497139834388898404L;
			...
			public Predicate toPredicate(Root<Reply> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if (searchMap.get("problemid") != null && !"".equals(searchMap.get("problemid"))) {
					predicateList.add(cb.like(root.get("problemid").as(String.class),
							"%" + (String) searchMap.get("problemid") + "%"));
				}
				// 回答内容
